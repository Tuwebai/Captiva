...
import { buildUniquenessFingerprint, estimateUniqueWordCount } from './content-uniqueness';
import { CITY_PAGE_SEO_RULES } from './seo-rules';

export type CityPageValidationResult = {
  valid: boolean;
  issues: string[];
};

export function validateCityPage(entry: CityLandingEntry): CityPageValidationResult {
  const issues: string[] = [];
