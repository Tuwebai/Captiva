import type { HomePlan } from './plan-catalog';
    ...
    >
      ...
      <div className="p-7">
        <div className="mb-6">
          ...
          <p className="text-sm text-zinc-400 mb-4">{plan.audience}</p>
        ...
        <ul className="space-y-3 mb-6">
          {plan.features.map((feature) => (
            <li key={feature} className="flex items-start gap-2.5 text-sm text-zinc-300">
              <CheckIcon />
              <span className={feature.includes('Todo lo de') ? 'font-semibold text-zinc-200' : ''}>{feature}</span>
            </li>
          ))}
        </ul>
