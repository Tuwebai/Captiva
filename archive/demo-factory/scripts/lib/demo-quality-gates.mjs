...
function countFaqItems(html) {
  const legacyItems = (html.match(/class="faq-item"/gi) ?? []).length;
  const cardItems = (html.match(/class="[^"]*faq-card[^"]*"/gi) ?? []).length;
  const detailsItems = (html.match(/<details\b/gi) ?? []).length;
  return Math.max(legacyItems, cardItems, detailsItems);
}
...
export function evaluateDemoQuality({ html, meta, designContract, demoName = 'unknown' }) {
  ...
  const faqItems = countFaqItems(html);
  ...
  if (gates.requiresTestimonialsInfiniteCarousel) {
    ...
    const hasApprovedAlternative =
      /class="testimonials-grid/i.test(html) ||
      /class="testimonials-strip/i.test(html) ||
      /class="testimonial-card/i.test(html);
    const testimonialCards =
      (html.match(/class="[^"]*testimonial-card[^"]*"/gi) ?? []).length +
      (html.match(/class="[^"]*marquee-card[^"]*"/gi) ?? []).length;
    const hasInfiniteCarousel = hasTrack && hasAnimation && hasClonedSlides;
    const hasStructuredAlternative = hasApprovedAlternative && testimonialCards >= 3;
    if (!hasInfiniteCarousel && !hasStructuredAlternative) {
